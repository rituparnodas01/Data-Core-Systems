...
app.put('/products/:id', verifyToken, upload.single('product_Image'), async (req, res) => {
  try {
    const { id } = req.params;
    const { product_name, product_description } = req.body;
    const productImage = req.file ? req.file.filename : null; // Get filename if file uploaded

    await productsModel.createProductTable(); // Wait for product table creation

    const updatedProduct = await productsModel.updateProduct(id, product_name, product_description, productImage); // Wait for product data update
