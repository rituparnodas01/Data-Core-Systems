...
function updateProduct(id, productName, productDescription, productImage) {
  return new Promise((resolve, reject) => {
    ...
    // Check if productImage is provided, otherwise set it to null
    ...
    connection.query(
      updateProductQuery,
      [productName, productDescription, image, id],
      (err, results) => {
        if (err) {
          console.error('Error updating product data:', err);
          reject(err);
...
function patchProduct(id, updates) {
  return new Promise(function(resolve, reject) {
    // Create an array to store the SET clauses
    var setClauses = [];
    var values = [];
    ...
    var setClause = setClauses.join(', ');

    // Construct the SQL query
    var patchProductQuery = `UPDATE products SET ${setClause} WHERE id = ?`;
    values.push(id);

    // Execute the SQL query
    connection.query(patchProductQuery, values, function(err, results) {
      if (err) {
        console.error('Error patching product data:', err);
        reject(err);
      } else {
